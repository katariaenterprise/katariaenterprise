            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {stats.map((s, i) => (
                <AnimatedSection key={s.label} delay={i * 0.1}>
                  <div className="flex flex-col items-center text-center gap-4 p-8 rounded-2xl bg-background hover-lift">
                    ...
                    <p className="font-heading font-black text-4xl text-foreground">{s.value}</p>
              ...
              {values.map((v, i) => (
                <AnimatedSection key={v.title} delay={i * 0.15}>
                  <div className="bg-card border border-border rounded-2xl p-8 text-center hover-lift h-full">
                    <div className="w-14 h-14 rounded-xl gradient-red flex items-center justify-center mx-auto mb-5">
                ...
                {milestones.map((m, i) => (
                  <AnimatedSection key={m.year} delay={i * 0.1}>
                    <div className="flex gap-6 items-start pl-2">
                      <div className="w-10 h-10 rounded-full gradient-red flex items-center justify-center shrink-0 z-10 border-4 border-background">
                        <span className="text-primary-foreground text-xs font-heading font-black leading-none text-center">{m.year.slice(0, 2)}</span>
                      </div>
                      <div className="bg-background border border-border rounded-2xl p-5 flex-1 hover-lift">
                        <p className="font-heading font-black text-primary text-lg mb-1">{m.year}</p>
